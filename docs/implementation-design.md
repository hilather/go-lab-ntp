...
| **D2** | First-party `internal/ntpwire` codec. **No** NTP library (`beevik/ntp`, `facebook/time`, chrony, ntpd). Direct deps: `gopkg.in/yaml.v3` (KnownFields) and `github.com/modelcontextprotocol/go-sdk v1.7.0` only. | NTP is a 48-byte header; LabDNS already hid wire types in `internal/dnswire`. A library would leak types and invite client-mode discipline we must not do. |
...
| **D16** | Operator UI ships after the data/control-plane PRs. Mira reviews the SPA. First UI: filter table, enable/disable, preview-an-IP, leap/stratum chips, live vs reset-only labeled. No JS heap browser for logs beyond the query ring. | FR. Implemented in PR 13; see [docs/12-web-ui.md](12-web-ui.md) and [docs/remaining-work.md](remaining-work.md) D29–D38. Do not rewrite D1–D28. |
...
- **Description:** Official SDK v1.7.0, protocol pin, `Stateless: true`, `ntp_*` tools (not `labntp_*`), `labntp://` resources. Bearer-only; fail closed with zero tokens (same as REST). MCP must not HTTP-call REST. `make test-parity`. `allowLegacyClients` default false. Freeze features id list in this PR.
